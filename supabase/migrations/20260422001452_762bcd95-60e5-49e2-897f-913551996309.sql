-- Fix update_lead_rpc to validate membership via org_members (not stale profiles.organization_id)
-- This ensures sellers can update leads in the active org and admins see those updates immediately.
...
DECLARE
  ...
  v_lead json;
BEGIN
  -- Resolve profile id (independent of stale profile.organization_id)
  ...
  -- Get lead's org and ownership info
  ...
  -- Validate user is an active member of the lead's organization
  ...
  -- Sellers can only update their own leads (responsible OR creator). Admins can update any.
  IF v_member_role <> 'admin' THEN
    IF v_lead_seller IS DISTINCT FROM v_profile_id
       AND v_lead_creator IS DISTINCT FROM v_profile_id::text
       AND v_lead_creator IS DISTINCT FROM p_clerk_user_id THEN
