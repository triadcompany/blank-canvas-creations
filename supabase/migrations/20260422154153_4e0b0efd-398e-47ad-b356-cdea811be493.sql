-- ── 1. Add group support columns to conversations ──
ALTER TABLE public.conversations
  ADD COLUMN IF NOT EXISTS is_group boolean NOT NULL DEFAULT false,
  ADD COLUMN IF NOT EXISTS group_name text,
  ADD COLUMN IF NOT EXISTS group_participants_count integer;

-- ── 2. Add sender info columns to messages (for group messages) ──
ALTER TABLE public.messages
  ADD COLUMN IF NOT EXISTS sender_name text,
  ADD COLUMN IF NOT EXISTS sender_phone text,
  ADD COLUMN IF NOT EXISTS sender_avatar_url text;

-- ── 3. Update get_org_conversations to expose group fields ──
...
-- ── 4. Update get_conversation_messages to expose sender fields ──
