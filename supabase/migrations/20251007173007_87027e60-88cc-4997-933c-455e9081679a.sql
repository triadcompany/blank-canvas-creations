-- Criar configurações de distribuição apenas para organizações que têm profiles
...
WHERE NOT EXISTS (
  SELECT 1 FROM lead_distribution_settings lds 
  WHERE lds.organization_id = o.id
)
GROUP BY o.id, p.id
HAVING p.id = (SELECT id FROM profiles WHERE organization_id = o.id ORDER BY created_at ASC LIMIT 1);

-- Atualizar função handle_new_user para criar configurações de distribuição
...
BEGIN
  ...
  ELSE
    ...
    -- Criar configurações de distribuição padrão
