-- Drop e recriar a função distribute_lead com lógica corrigida
DROP FUNCTION IF EXISTS public.distribute_lead(uuid, uuid);
...
BEGIN
  -- Advisory lock específico para esta organização
  ...
  -- Verificar se já existe atribuição (idempotência)
  SELECT EXISTS(
  ...
  -- Carregar configurações COM LOCK
  ...
  -- Determinar usuário baseado no modo
  ...
  -- Criar atribuição (unique constraint garante unicidade)
  ...
  -- Chamar edge function para notificação via WhatsApp (assíncrono)
