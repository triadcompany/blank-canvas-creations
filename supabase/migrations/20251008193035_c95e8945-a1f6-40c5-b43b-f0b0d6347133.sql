...
-- Fix distribute_lead function to properly implement round-robin logic
...
BEGIN
  ...
  ELSIF v_settings.mode = 'auto' THEN
    ...
    -- Log para debug
    RAISE NOTICE 'Round-robin: users=%, cursor=%, selected_user=%, next_cursor=%', 
      v_user_count, v_cursor, v_assigned_user_id, v_next_cursor;
  ...
  VALUES (
    ...
    jsonb_build_object(
      ...
      'cursor_used', COALESCE(v_cursor, 0),
      'next_cursor', COALESCE(v_next_cursor, 0),
      'timestamp', now()
    )
  );
